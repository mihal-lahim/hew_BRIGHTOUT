...
#include "../include/WICTextureLoader11.h"
...
namespace hal
{
	...
	{
		m_TextLines.back().strings.emplace_back(color); //�ǉ�������o�b�t�@

		for(; *pText; pText++) {

			if (*pText == '\n') {
				m_TextLines.emplace_back(); // ���s
				m_TextLines.back().strings.emplace_back(color); // �V�����s�̕�����̈�
			}
			else if (*pText == '\r') {
				// �L�����b�W���^�[���͖���
			}
				...
				while (m_TextLines.back().characterCount %4 !=0) {
					if (m_MaxCharactersPerLine && m_TextLines.back().characterCount >= m_MaxCharactersPerLine) {
						m_TextLines.emplace_back(); // ���s
						// m_TextLines.back().strings.emplace_back(color); //�����ł͋󔒑}���̂�
						break;
					}
		...
		while(m_MaxLine && m_TextLines.size() + last_line_count > m_MaxLine) {
			...
			m_CharacterCount -= remove_character_count; // �S�̕������𒲐�
			m_TextLines.pop_front(); //�擪�s���폜
		}
	}
