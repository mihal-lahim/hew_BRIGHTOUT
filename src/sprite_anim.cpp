...
{
	m_accumulated_time += elapsed_time; // �ݐώ��Ԃ��X�V
	if (m_accumulated_time >= m_pAnimPattern->GetSecoundPerPattern())
	{
		m_pattern = (m_pattern + 1) % m_pAnimPattern->GetPatternMax(); // �p�^�[���ԍ����X�V
		m_accumulated_time -= m_pAnimPattern->GetSecoundPerPattern();  // �ݐώ��Ԃ����Z�b�g
	}
}
